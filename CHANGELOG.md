...
- Verification-preserving test seeding: `Chronicle\Testing\LedgerSeeder` records N entries through the real write path inside a single transaction and writes periodic signed checkpoints (`->checkpointEvery(K)`), yielding a chain that passes both `IntegrityVerifier::verify()` and `CheckpointChainVerifier::verify()` with every entry anchored. Returns a `SeededLedger` summary (`entries`, `checkpoints`, `lastCheckpointId`). Test-only (shipped under the `Testing` namespace), with no impact on production code paths.
...
- `PayloadCipher` + `CipherEnvelope`: AEAD encrypt/decrypt of payload field sets under a subject DEK using libsodium XChaCha20-Poly1305-IETF (decision D5) with a fresh per-call 192-bit nonce and the canonical entry envelope `(id, subject_type, subject_id, action)` as Associated Data - so ciphertext cannot be transplanted between entries. Decryption fails loudly on a wrong DEK, AAD mismatch, or tampered ciphertext. The self-describing `{_chronicle_enc:'v1', nonce, ciphertext}` envelope makes encrypted fields recognisable on read.
- `EncryptPayload` pipeline stage (runs between `CanonicalizePayload` and `HashPayload`, in both the synchronous and queued-driver pipelines): when `chronicle.encryption.enabled` is on, it encrypts the configured fields (default `metadata`/`context`/`diff`) under the subject DEK and writes the *same* cipher envelope into both the hashed `payload` JSON and the denormalized columns, so `payload_hash`/`chain_hash` now cover ciphertext while the v1.11 `ColumnPayloadDivergence` check still passes. Empty/null fields stay cleartext; entries without a subject are skipped. Disabled => behaviour identical to 1.11.
- Decrypt-on-read: `Entry::decryptedMetadata()`/`decryptedContext()`/`decryptedDiff()` transparently decrypt encrypted fields when the subject DEK exists, return the value unchanged when it was never encrypted, and return a typed tombstone `['_erased' => true, 'erased_at' => ...]` once the DEK has been destroyed. Backed by a read-only `SubjectKeyManager::stateFor()` (never mints a key for an erased/unknown subject), a `SubjectKeyState` value object, and an `EntryDecryptor` kept separate from the model's attribute accessors so verification still sees only the raw stored envelope. Cleartext columns (actor/action/subject/tags/time) are never encrypted, so query filters and trail listings still work for erased entries.
...
- `chronicle:encrypt-backfill {--from=} {--chunk=500} {--dry-run} {--force}`: a deliberate, one-off re-baselining migration that encrypts the configured fields of historical entries via `EncryptBackfiller`, then creates a fresh signed checkpoint at the new head and logs the re-baseline. Prints a loud "take a backup first" banner, refuses to run in production without `--force` (and prompts for a backup confirmation otherwise), and runs the rewrite + checkpoint in a single transaction. `--dry-run` reports scope without writing; an already-encrypted ledger is a no-op with no new checkpoint. Never wired into normal recording.
