...
- Verification-preserving test seeding: `Chronicle\Testing\LedgerSeeder` records N entries through the real write path inside a single transaction, yielding a chain that passes `IntegrityVerifier::verify()`. Test-only (shipped under the `Testing` namespace), with no impact on production code paths.
